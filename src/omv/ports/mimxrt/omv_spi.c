...
static void spi_master_callback(LPSPI_Type *base, void *handle, status_t status, void *user) {
    ...
    if ((spi->dma_mode & OMV_SPI_HALF_CB_EN) && ((spi->xfer_flags & flags) == flags)) {
    ...
    if ((spi->dma_mode & OMV_SPI_DMA_CIRCULAR) && ((spi->xfer_flags & flags) == flags)) {
...
int omv_spi_transfer_start(omv_spi_t *spi, omv_spi_transfer_t *xfer) {
    ...
    omv_spi_set_frame_size(spi, xfer->datasize);
    ...
    if (spi->dma_mode & OMV_SPI_HALF_CB_EN) {
        spi->xfer_descr.dataSize /= 2;
    }

    // Check if we can optimize the transfer by doing things in 32-bit chunks.
    if ((!(spi->xfer_descr.dataSize % 4)) && (xfer->datasize == 8)) {
...
static int omv_spi_dma_init(edma_handle_t *dma_handle, const dma_descr_t *dma_descr) {
    ...
    DMAMUX_SetSource(DMAMUX, channel, dma_descr->request);
    DMAMUX_EnableChannel(DMAMUX, channel);
    EDMA_CreateHandle(dma_handle, DMA0, channel);
    return 0;
}
...
int omv_spi_init(omv_spi_t *spi, omv_spi_config_t *config) {
    ...
    // Configure pins.
    mimxrt_hal_spi_init_deinit(config->id, config->nss_enable, config->nss_pol, true);
...
int omv_spi_deinit(omv_spi_t *spi) {
    if (spi && spi->initialized) {
        ...
        omv_spi_transfer_abort(spi);
        if (spi->dma_mode & (OMV_SPI_DMA_NORMAL | OMV_SPI_DMA_CIRCULAR)) {
            DMAMUX_DisableChannel(DMAMUX, spi->dma_descr_tx.channel);
            free_dma_channel(spi->dma_descr_tx.channel);
            DMAMUX_DisableChannel(DMAMUX, spi->dma_descr_rx.channel);
            free_dma_channel(spi->dma_descr_rx.channel);
        }
        LPSPI_Deinit(spi->inst);
        mimxrt_hal_spi_init_deinit(spi->id, false, false, false);
    }
    return 0;
}
