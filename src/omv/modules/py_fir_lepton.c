...
static void fir_lepton_spi_callback(omv_spi_t *spi, void *data);
...
void fir_lepton_spi_callback(omv_spi_t *spi, void *data) {
    const uint16_t *base = (uint16_t *) &fir_lepton_buf;

    if (!(spi->xfer_flags & OMV_SPI_XFER_HALF)) {
        base += VOSPI_PACKET_SIZE;
    }
...
int fir_lepton_init(omv_i2c_t *bus, int *w, int *h, int *refresh, int *resolution) {
    ...
    spi_config.dma_mode = OMV_SPI_DMA_CIRCULAR | OMV_SPI_HALF_CB_EN;
    spi_config.dma_size = VOSPI_BUFFER_SIZE;
